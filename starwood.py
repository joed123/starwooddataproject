...
def find_last_end_of_section(pdf_document, start_page, target_count):
    ...
    end_section_page = None
    print("made it intot eh fucntion")
    ...
    for page_num in range(start_page, len(pdf_document)):
        ...
        if "END OF SECTION" in text:
            ...
            if end_of_section_count >= target_count:
                ...
                print(f"ends on {end_section_page}")
...
def find_section_title(pdf_path):
    ...
    if next_line:
        ...
        print("Found Next Line:", next_line)
        print("Formatted Section Title:", section_title)
    else:
        print("'DIVISION 08 -- OPENINGS' not found.")
        pdf_document.close()
        return
    ...
    if matching_page:
        print(
            ...
            f"found on page {matching_page}."
        )
    else:
        print(f"Section Title '{section_title}' not found in the document.")

    print(f"division_08_count: {division_08_count}")
    begin_section_page = matching_page
    end_section_page = find_last_end_of_section(
        pdf_document, matching_page, division_08_count)
    print(end_section_page)
    pdf_document.close()
